...
class PDFGrabber():
    ...
    def get_keys(self):
        ...
        for agreement in agreement_list:
            ...
            with urllib.request.urlopen(f'https://assist.org/api/agreements?receivingInstitutionId={self.school_id}&sendingInstitutionId={school_id}&academicYearId={year}&dept=major') as url:
                data = json.loads(url.read().decode())
            data = data['reports']
